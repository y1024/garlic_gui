...
- 导出不再因为个别类名大小写冲突，把整包写进 `_classes/` 下的十六进制路径。
  Java / Smali 按包目录和类名输出，例如 `com/zhihu/android/Foo.java`。
- 只有真正会写到同一路径的文件才在文件名后追加 `~` 和短哈希，避免大小写不敏感的文件系统互相覆盖。
  内部类仅大小写不同时，不再把外层类一起编码。后台缓存仍使用十六进制路径。

## 0.5.43 搜索横向滚动与引用参数 null

- 项目搜索结果按内容撑开列宽，底部始终显示横向滚动条，长类名可以横向拖看。
- Dalvik `const/4 0` 作为对象参数（如 `HashMap`）时，调用点输出 `null`，与方法声明类型一致，不再显示成 `int` 的 `0`。

## 0.5.42 MCP 进程回收与搜索主题

- 无头 stdio MCP 在自身事件循环中处理输入，不再为每次启动额外创建桥接进程。GUI stdio 桥接在服务断开后及时退出，包括等待输入或仅收到半行输入时；输入队列有大小限制。
- 无头模式响应 SIGTERM / SIGINT，退出时取消分析并回收引擎。客户端应在同一 MCP 会话上连续调用工具，结束时关闭 stdin；不要每次工具调用都启动一个新服务。
- HTTP 默认绑定 `127.0.0.1`。GUI 可在首选项修改“HTTP 绑定 IP”，无头模式新增 `--http-host IP`（与 `--http-port` 一起使用）。
- 修复切换回深色主题后，搜索结果交替行显示白底的问题。

## 0.5.41 构造引用调用点定位

- 为反编译器转换后的 `INITIALIZE` 表达式保留构造函数语义映射。查询 `n.<init>` 的引用并双击结果时，会选中 `new n(...)` 中实际被引用的 `n`。
- 构造函数和静态初始化函数不再沿继承关系归一到父类同签名函数，避免混淆短类名的构造引用跳到错误目标。

## 0.5.40 构造调用引用定位

- Java 源码位置映射会将构造调用 `new Type(...)` 关联到 `<init>` 的语义标识。查找构造函数引用后双击结果会选中实际调用处的 `Type`，而不是只打开文件或停留在错误位置。

## 0.5.39 构造函数引用定位

- 引用结果保存来源方法的作用域。双击构造函数内的引用时，编辑器会在该构造函数中选中对应调用，不会跳到同一类中其他相同目标的调用。
...
- “与编辑器同步”改为按当前光标所在的方法或字段同步。光标落在函数调用、局部变量或普通语句时，树会选中其所属函数，不再跟随被引用的函数。
...
- 打开项目搜索时，仅在编辑器存在选区时将选中文本带入搜索框；没有选区时保留当前搜索条件。
- “与编辑器同步”现在会展开并选中当前函数或字段；局部变量同步到所属函数。成员索引未就绪时会等待完成后执行。
- DEX/APK 的嵌套及匿名类按其完整名称单独请求 Java 源码并使用独立缓存，不再错误复用外层类。Smali 行为保持不变。

## 0.5.34 独立反混淆选项与指令定位

- 反混淆拆成“函数和变量名称反混淆（含类名）”及“字符串反混淆”，默认均关闭；字符串静态还原独立运行，不依赖名称改名。
- 反编译新增数值格式 `auto`、十进制、十六进制，适用于整数、长整数及 switch 分支值。auto 对较大的 2 的幂和位掩码使用十六进制，其余保留十进制；布尔值和浮点值保持原有输出规则。
- 标题显示“版本号 - 输入文件名”，取消文件名单独占行，包/类数量移到工具栏右侧。
- Java 源码映射新增语句/条件表达式的 DEX 指令偏移与所属方法，Tab 在 if、switch、return 等非符号位置也能定位；空白或被优化的指令选择同一方法中最近的可用语句。映射随改名注释和显示格式同步移动。
- 无头模式新增 `--deobfuscate-strings` 和 `--number-format auto|decimal|hex`；新配置分别保存和区分源码缓存。

## 0.5.33 Java / Smali 切换与搜索

- 代码区按 Tab 切换 Java / Smali，按所属方法及符号出现位置恢复光标；目标源码未加载时在加载完成后定位。两种表示无法一一对应时定位所属声明。
- 字节码引用结果优先按需加载目标 Java 类，在所属方法内查找引用；被反编译优化掉的表达式会提示并定位所属方法。
- 搜索新增“Smali 代码”，默认关闭并记忆勾选状态。开启后额外搜索完整 Smali，后台生成可取消，生成结果在当前打开的项目中复用；默认 Java 搜索不会生成 Smali。

## 0.5.32 改名来源与默认包筛选

- 类、方法、字段和局部变量改名后，在声明行上方显示 `renamed from: 原名, reason: 原因`，支持自动反混淆、手动改名、映射导入；原因随项目和索引缓存保存，撤销时同步恢复。
- 参考 jadx 的分类序号和长度规则，保留稳定的 `Class_` / `method_` / `field_` / `local_` 命名；类和成员识别短名、超过 64 字符的名称、Java 保留字、非 ASCII/非法标识符和易混淆字符。构造方法随所属类改名，已有别名优先保留。
- 包限制支持 `<default>` 和 `默认包`，只匹配无包路径的类；默认包右键搜索自动填写 `<default>`，留空仍搜索所有包。

## 0.5.31 搜索历史与注解引用

- 搜索文本下拉框保留最近 50 条去重历史，新查询挤出最旧记录。
- DEX 索引包含类、字段、方法、参数及嵌套注解中的类型/成员引用与泛型签名类型；源码支持限定名注解定位。
- 反混淆覆盖 Unicode 乱码字段和 O0o/Il1 型名称，局部变量使用稳定别名；字段/方法签名显示类型别名。
- 修改反混淆、控制流整理或反平坦化选项后询问是否立即重建；取消保留当前分析结果。树选择不绘制焦点虚线。

## 0.5.30 导航历史、Hex 与字节码定位

- 历史记录包含文件、Java/Smali 模式、光标选择及滚动位置；支持同文件内跳转和鼠标侧键，后退后的新跳转清除前进分支。
- Hex 按固定字节格绘制并强制等宽字体，恢复浅色/深色字节配色；支持拖选、Shift 扩选、全选，以及文本、Hex、Base64 复制。预览仍按需加载，单次复制限制 64 MiB。
- DEX Smali 输出指令偏移标记；尚无 Java 定位信息的字节码引用直接定位到对应 Smali 指令。

## 0.5.29 双击无定义单词保持原位

- 移除无符号单词的正则猜测跳转，双击关键字、未解析类型或无定义名称时只选中单词。
- 局部变量使用精确符号映射定位；跨类跳转先确认项目中存在定义，避免打开无关类或留下无效跳转请求。
...
提供 Streamable HTTP 的 POST / JSON 响应，GET 返回 405。不需要 auth。默认绑定 `127.0.0.1`，用户可在首选项修改。绑定 IP 可设为 `0.0.0.0` 或 `::`，局域网客户端使用本机局域网 IP。
