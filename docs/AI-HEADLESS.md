...
在同一会话上连续调用工具，不要为每次工具调用启动新进程；无头 stdio 服务不再派生桥接子进程。
GUI 选中文本/当前类工具在无头模式不可用，请明确传入 `class_name`。

需要常驻 HTTP 服务时，用 `--headless --apk PATH --http-port 8650` 启动，
默认仅绑定 `127.0.0.1`，连接 `http://127.0.0.1:8650/mcp`；端口 `0` 自动分配，实际 URL 写入标准错误。
可通过 `--http-host 0.0.0.0` 或其他 IP 修改绑定地址（仅用于 HTTP 模式）。
